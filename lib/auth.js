    ...
    configure: function configure(i18n) {
        passport.use(
            ...
            new FacebookStrategy({
                ...
                profileFields: ['id', 'emails', 'username', 'name', 'first_name', 'last_name', 'link']
            },
    ...
    init: function init(server) {
        if (server.get('authInitialized') !== true) {
            ...
            server.use(function(req, res, next) {
                ...
                /*if (req.user.blocked_words && req.user.blocked_words.length) {
                    if (server.locals.siteSettings && server.locals.siteSettings.blockedWords) {
                        server.locals.siteSettings.blockedWords += ' ' + ('' + req.user.blocked_words).trim();
                    }
                }*/
